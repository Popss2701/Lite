...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
            'categories_description':{
                'type':'select',
                'query': "SELECT * FROM _DBPRF_categories_description WHERE categories_id IN " + self.list_to_in_condition(category_ids)
            },
		}
		categories_ext = self.select_multiple_data_connector(categories_ext_queries, 'categories')
		languages_ids = duplicate_field_value_from_list(categories_ext['data']['categories_description'], 'language_id')
		categories_ext_rel_queries = {
			'languages': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_languages WHERE languages_id IN " + self.list_to_in_condition(languages_ids)
			}
		}
	...
	def convert_category_export(self, category, categories_ext):
		...
		# img = get_row_from_list_by_field(category['data']['categories_image'],'categories_image', category['categories_id'])
		...
		category_data['name'] = categories_ext['data']['categories_description'][0]['categories_name']
		category_data['meta_title'] = ''
		category_data['meta_keyword'] = ''
		# meta_description
		category_data['language'] = categories_ext['data']['languages'][0]
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			'products_description': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_products_description WHERE products_id IN " + self.list_to_in_condition(product_ids)
			},
			'products_categories': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_products_to_categories WHERE products_id IN " + self.list_to_in_condition(product_ids)
			}
		...
		languages_ids = duplicate_field_value_from_list(product_ext['data']['products_description'], 'language_id')
		categories_ids = duplicate_field_value_from_list(product_ext['data']['products_categories'], 'categories_id')
		product_ext_rel_queries = {
			...
			'categories': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_categories WHERE categories_id IN " + self.list_to_in_condition(categories_ids)
			}
	...
	def convert_product_export(self, product, products_ext):
		...
		if to_int(product['products_quantity'])>=0:
			product_data['qty'] = product['products_quantity']
		else:
			product_data['is_in_stoke']=False
			product_data['qty'] = 0
		...
		product_data['updated_at'] = product['products_last_modified'] if product['products_last_modified'] else get_current_time()
		product_data['name'] = products_ext['data']['products_description'][0]['products_name']
		product_data['description'] = html.unescape(products_ext['data']['products_description'][0]['products_description'])
		# product_description = get_list_from_list_by_field(products_ext_data['products_meta'], 'products_id',product['id'])
		...
		url_product_image = 'https://www.iemotorsport.com/wp-content/uploads/2020/05'
		product_path = 'BMW-2002-TI-TII-M10-HOT-WHEELS-DIECAST-CAR-IRELAND-ENGINEERING-MOTORSPORTS-OEM-GENUINE-TURBO-ALPINA-PERFORMANCE-RACE-RACING-CUSTOM-2-scaled.jpg'
		# url_product_image= 'http://localhost/wordpress/wp-content/uploads/2020/09/'
		...
		if product['products_image']!='':
			...
			product_data['thumb_image']['path'] = product_path
		product_image = product['products_image_large']
		if product_image:
			if check_thumbnail:
				product_image_data = self.construct_product_image()
				product_image_data['url'] = url_product_image
				product_image_data['path'] = product_path
		...
		product_categories = get_list_from_list_by_field(products_ext_data['products_categories'], 'products_id', product['products_id'])
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		customer_data['username'] = customer['customers_lastname']
		...
		customer_data['is_subscribed'] = True if to_int(customer['customers_newsletter']) == 1 else False
		customer_data['phone'] = customer['customers_telephone']
		customer_data['fax'] = customer['customers_fax']
		customer_data['active'] = True
		# customer_data['created_at'] = customers_ext['data']['customers_info'][0]['customers_info_date_account_created']
		# customer_data['updated_at'] = customers_ext['data']['customers_info'][0]['customers_info_date_account_last_modified'] if customers_ext['data']['customers_info'][0]['customers_info_date_account_last_modified'] else get_current_time()
		...
		if customer_info:
			...
			customer_data['updated_at'] = customer_info['customers_info_date_account_last_modified'] if customer_info['customers_info_date_account_last_modified'] else get_current_time()
	...
	def get_orders_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM  _DBPRF_orders WHERE orders_id > " + to_str(id_src) + " ORDER BY orders_id ASC LIMIT " + to_str(limit)
		}
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			...
			'zones': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_zones WHERE zone_name IN " + self.list_to_in_condition(state_ids) + " OR zone_code IN " + self.list_to_in_condition(state_ids)
			},
			'orders_customer': {
				'type': 'select',
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_total = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'orders_id', order['orders_id'])
		...
		if ot_subtotal:
			order_data['total']['title'] = ot_subtotal['title']
			order_data['total']['amount'] = ot_subtotal['value']
		...
		customer_address['first_name'] = order['delivery_name'].split(' ')[0]
		customer_address['last_name'] = order['delivery_name'].split(' ')[1]
		...
		customer_address['country'] = order['delivery_country']
		...
		billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_name', order['billing_state'])
		if billing_state:
		...
		order_delivery['first_name'] = order['delivery_name'].split(' ')[0]
		order_delivery['last_name'] = order['delivery_name'].split(' ')[1]
		...
		order_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name',order['delivery_country'])
		if order_country:
			order_delivery['country']['id'] = order_country['countries_id']
			order_delivery['country']['code'] = order_country['countries_iso_code_2']
			order_delivery['country']['country_code'] = order_country['countries_iso_code_2']
			order_delivery['country']['name'] = order_country['countries_name']

		order_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_name', order['delivery_state'])
		if order_state:
			order_delivery['state']['id'] = order_state['zone_id']
			order_delivery['state']['state_code'] = order_state['zone_code']
			order_delivery['state']['name'] = order_state['zone_name']
		...
		#payments = get_row_from_list_by_field(orders_ext['data']['orders_payment'], 'order_id', order['orders_id'])
		...
		for order_product in order_products:
			...
			order_item['tax_amount'] = order_product['products_tax']
			...
			order_item['total'] = to_decimal(order_item_subtotal) + to_decimal(order_product['products_tax'])
